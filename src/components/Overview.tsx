        ...
        <section className="overview" aria-labelledby="overview-title">
            ...
            <div className="overview-body">
                <div className="overview-panel" id="protocol" role="tabpanel" aria-labelledby="tab-protocol" tabIndex={0} hidden={activeTab !== 'protocol'}>
                    <div className="feature-grid">{features.map(({ title, description, icon: Icon }) => (<article className="feature" key={title}><div className="feature-top"><span className="feature-icon"><Icon size={23} strokeWidth={1.5} aria-hidden="true" /></span></div><h3>{title}</h3><p>{description}</p></article>))}</div>
                </div>
